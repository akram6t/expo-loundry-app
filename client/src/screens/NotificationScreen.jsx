  ...
  function getNotifications() {
    ...
    axios.get(`${server.baseUrl}/${api.notifications}/${uid}?skip=${skip}`, { headers: { "Content-Type": 'application/json', apikey: server.apikey } })
        .then((result, err) => {
            ...
            if (status) {
                setSkip(skip + 10);
                setNotificationsList([...ordersList, ...data]);
                if(data.length===0){
                    setListEnd(true);
                }
            }
...
const onRefresh = React.useCallback(() => {
  ...
  function NotificationItem({ item }) {
    const title = `${item?.type === 'order' ?  `Order Id #${item.title}` : item.title}`
    const message = `${item.type === 'order' ? 'Your order ' + item.message : item.message }`

    return (
      <View style={{ margin: 5, borderRadius: 10, borderWidth: 1, borderColor: MD2Colors.grey300, paddingHorizontal: 5, paddingVertical: 8, backgroundColor: item.color ? item.color+'2A' : 'white', flexDirection: 'row', justifyContent: 'space-between'}}>

        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
          <Image style={{ width: 50, height: 50 }} source={ !item.icon ? require('./../../assets/images/forground_logo.png') : { uri: server.baseUrl + item.icon }} />
          <View>
...
<View style={{ flex: 1 }}>
        ...
        <SafeAreaView style={{ flex: 1, backgroundColor: MD2Colors.grey200 }}>
            ...
            <FlatList
                ...
                <Button disabled={listEnd} onPress={() => getNotifications()} loading={loading}>{!listEnd ? 'load more' : 'No orders available'}</Button>
